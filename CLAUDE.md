...
OTP/Elixir versions are declared in several places that must move together when bumped. **`.tool-versions` is the source of truth**; `jose` requires OTP 27 (native `json` module, `dynamic()` type), so none of these may drop below OTP 27:

- **`.tool-versions`** — local dev (asdf); source of truth (`erlang`, `elixir`, `nodejs`).
- **`nixpacks.toml`** — production build (`beam.packages.erlang_27.elixir_1_19`, `nodejs_22`) + its pinned `nixpkgsArchive`.
- **`.github/workflows/test-coverage.yml`** — CI (`ELIXIR_VERSION` / `OTP_VERSION` env).
- **`.github/workflows/copilot-setup-steps.yml`** and **`.github/workflows/shared/elixir-setup.md`** — GitHub Copilot Agent (`erlef/setup-beam` pins; the `.md` is the canonical template).
