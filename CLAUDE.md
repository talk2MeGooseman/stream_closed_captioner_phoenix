...
- **Admin is gated by `user.uid == "120750024"`** via the `:admin_protected` pipeline (protects `/live-dashboard`, `/feature-flags`, and the `/admin/*` LiveViews including `/admin/local-extension-testing`).
- **Local extension testing.** `/admin/local-extension-testing` (admin-only) lists currently-live caption channels with one-click links that carry a short-lived **per-channel** socket token (GraphQL resolvers read the token's `channel_id` claim, not the query arg), the channel id, and this deploy's origin in the URL fragment, so the maintainer can drive the Twitch extension dev build locally against real captions. The local origin must be allowlisted via the `LOCAL_EXT_TESTING_ORIGINS` env var (comma-separated, full `http(s)://` origins — validated at boot), which `config/runtime.exs` appends to the websocket `check_origin` list and exposes to the GraphQL CORS allowlist (`StreamClosedCaptionerPhoenixWeb.CORS`) — off by default.
