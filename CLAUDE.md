...
- **Local extension testing.** `/admin/local-extension-testing` (admin-only) mints a short-lived socket token and lists currently-live caption channels so the maintainer can drive the Twitch extension dev build locally against real captions. The local origin must be allowlisted via the `LOCAL_EXT_TESTING_ORIGINS` env var (comma-separated), which `config/runtime.exs` appends to the websocket `check_origin` list — off by default.
