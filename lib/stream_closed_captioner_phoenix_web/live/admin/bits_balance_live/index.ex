defmodule StreamClosedCaptionerPhoenixWeb.Admin.BitsBalanceLive.Index do
  ...
  defp apply_action(socket, :index, _params) do
    assign(socket, page_title: "Bits Balances", record: nil)
  end

  defp apply_action(socket, :new, _params) do
    socket |> assign(page_title: "New Bits Balance", record: %BitsBalance{})
  end

  defp apply_action(socket, :edit, %{"id" => id}) do
    socket |> assign(page_title: "Edit Bits Balance", record: Admin.get_bits_balance!(id))
  end
  ...
  def handle_event("delete", %{"id" => id}, socket) do
    {:ok, _} = Admin.delete_bits_balance(Admin.get_bits_balance!(id))
    {:noreply, socket |> put_flash(:info, "Deleted.") |> load_records()}
  end

  defp load_records(socket) do
    %{search: s, page: page} = socket.assigns
    count = Admin.count_bits_balances(s)

    assign(socket,
      records: Admin.list_bits_balances(s, page),
      total_count: count,
  ...
  def render(assigns) do
    ~H"""
    <.admin_page_header title="Bits Balances" count={@total_count}>
      <:actions>
        <.admin_button patch={~p"/admin/bits-balances/new"}>New Bits Balance</.admin_button>
      </:actions>
    </.admin_page_header>
